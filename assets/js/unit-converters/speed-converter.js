document.addEventListener('DOMContentLoaded', function() {
    // Get DOM elements
    const form = document.getElementById('speedConverterForm');
    ...
    // Base unit conversion factors (to meters per second)
    const conversionFactors = {
        kmh: 0.277778, // km/h to m/s
        mph: 0.44704,  // mph to m/s
        ms: 1,         // m/s to m/s
        knots: 0.514444, // knots to m/s
        fps: 0.3048,   // ft/s to m/s
        mach: 340.3,   // Mach to m/s
        light: 299792458, // Speed of light to m/s
        sound: 343     // Speed of sound to m/s
    };
    ...
    function convert(value, fromUnit, toUnit) {
        try {
            ...
            const valueInMetersPerSecond = value * conversionFactors[fromUnit];
            // Convert from base unit to target unit
            return valueInMetersPerSecond / conversionFactors[toUnit];
        } catch (error) {
